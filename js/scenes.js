function text_box(text,width,debug){
  this.text = text ;
  this.x = -1 ;
  this.y = -1 ;
  this.w = width ;
...
function gathering_object(node, mode){
  this.read_from_xml = function(element){
    this.fields = get_fields_from_xml(element) ;
    this.characters = get_array_from_xml(element, 'character') ;
    this.items      = get_array_from_xml(element, 'item'     ) ;
  ...
  this.set_date_time = function(start_date,end_date){
    this.fields['start_date'] = start_date ;
    this.fields['end_date'  ] =   end_date ;
    this.start_year  = parseInt(this.fields['start_date'].split('/')[0],10) ;
    this.start_month = parseInt(this.fields['start_date'].split('/')[1],10) ;
    this.start_day   = parseInt(this.fields['start_date'].split('/')[2],10) ;
    this.end_year    = parseInt(this.fields[  'end_date'].split('/')[0],10) ;
    this.end_month   = parseInt(this.fields[  'end_date'].split('/')[1],10) ;
    this.end_day     = parseInt(this.fields[  'end_date'].split('/')[2],10) ;
  }
  this.set_date_time(this.fields['date'],this.fields['time']) ;
  ...
  this.arrange_items = function(){
    ...
    var url = (this.video) ? this.video.video_still_url(this.fields['video_still_id']) : '' ;
    this.youtube = new image_box(url,this.image_width,this.image_height, this) ;
    if(this.fields['thumbnail']) this.youtube = new image_box('images/cropped/'+this.fields['thumbnail'],this.image_width,this.image_height, this) ;
    this.youtube.x = this.w - scene_padding - this.image_width ;
    this.youtube.y = scene_padding ;
    ...
    this.image_width = 0 ;
    var date_string = (this.fields['date_string']==undefined) ? this.fields['start_date'] : this.fields['date_string'] ;
    this.video_text       = new text_box(this.fields['video_title' ], this.w-3*scene_padding-this.image_width) ;
    this.date_text        = new text_box(date_string                , this.w-3*scene_padding-this.image_width) ;
    this.filmed_text      = new text_box(this.fields['cameraperson'], this.w-3*scene_padding-this.image_width) ;
    this.place_text       = new text_box(this.fields['place'       ], this.w-3*scene_padding-this.image_width) ;
    this.description_text = new text_box(this.fields['description' ], this.w-2*scene_padding-this.image_width, true) ;
    this.tapetag_text     = new text_box(tape_tag_descriptions[this.fields['tape_tag']], this.image_width-6) ;
    
    this.video_text.h = 1*scene_icon_width ;
    var text_dy = 2 ;
    var w  = 0.6*(this.w - this.image_width - 4*scene_padding) ;
    var x1 = scene_padding ;
    var x2 = x1 + scene_padding + scene_icon_width ;
    ...
    this.place_icon.x  = x1 ;
    this.place_text.x  = x2 ;
    this.place_icon.y  = y1 ; //y3 ;
    this.place_text.y  = y2+4 ; // y4+4 ;
  ...
  this.draw = function(y_in,draw){
    var dy = 0 ;
    this.h = Math.max(this.h,2*scene_padding) ;
    ...
    dy += line_height ;
    if(this.video) this.video.draw(y,draw) ;
